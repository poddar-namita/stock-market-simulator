...
exports.purchaseStock = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, {
            balance:
                Math.round((user.balance - totalPrice + Number.EPSILON) * 100) /
                100,
        });
        ...
        return res.status(200).json({
            ...
            user: {
                ...
                balance:
                    Math.round(
                        (user.balance - totalPrice + Number.EPSILON) * 100
                    ) / 100,
            },
        });
    } catch (error) {
...
exports.sellStock = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, {
            balance:
                Math.round((user.balance + saleProfit + Number.EPSILON) * 100) /
                100,
        });
        ...
        return res.status(200).json({
            ...
            user: {
                ...
                balance:
                    Math.round(
                        (user.balance + saleProfit + Number.EPSILON) * 100
                    ) / 100,
            },
        });
    } catch (error) {
